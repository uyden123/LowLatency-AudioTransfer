...
#define SHARED_BUFFER_SIZE (44100 * 1 * 2 * 2) // 2 seconds of 16-bit Mono at 44.1kHz
static BYTE g_SharedAudioBuffer[SHARED_BUFFER_SIZE] = {0};
static volatile ULONG g_SharedAudioWriteOffset = 0;
static volatile ULONG g_SharedAudioReadOffset = 0;

// Global function to push data from Sideband/IOCTL directly to Mic
extern "C" void PushMicData(PBYTE Data, ULONG Size)
{
    for (ULONG i = 0; i < Size; i++)
    {
        g_SharedAudioBuffer[(g_SharedAudioWriteOffset + i) % SHARED_BUFFER_SIZE] = Data[i];
    }
    g_SharedAudioWriteOffset = (g_SharedAudioWriteOffset + Size) % SHARED_BUFFER_SIZE;
}
...
)
{
    ...
    if (!m_bCapture && (!g_DoNotCreateDataFiles))
    {
        ...
        // Virtual Mic uses following buffer to hold data before writing to a file.
    ...
    switch (State_)
    {
        ...
        case KSSTATE_RUN:
            ...
            if (m_ulNotificationIntervalMs > 0)
            {
                ...
                // notification events only after notification interval. This timer is used by Virtual Mic to 
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ...
        ULONG available = (g_SharedAudioWriteOffset - g_SharedAudioReadOffset + SHARED_BUFFER_SIZE) % SHARED_BUFFER_SIZE;
        // If capture is too far behind, skip to latest data
        if (available > SHARED_BUFFER_SIZE / 2) {
            g_SharedAudioReadOffset = g_SharedAudioWriteOffset;
            available = 0;
        }
        
        for (ULONG i = 0; i < runWrite; i++)
        {
            if (i < available) {
                m_pDmaBuffer[bufferOffset + i] = g_SharedAudioBuffer[(g_SharedAudioReadOffset + i) % SHARED_BUFFER_SIZE];
            } else {
                m_pDmaBuffer[bufferOffset + i] = 0; // silence if no data
            }
        }
        
        ULONG consume = min(runWrite, available);
        g_SharedAudioReadOffset = (g_SharedAudioReadOffset + consume) % SHARED_BUFFER_SIZE;
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ...
        // Removed: g_SharedAudioBuffer write logic. 
        // Speaker now works independently as requested.
...
--*/
{
    ...
    //
    // Virtual Mic writes each stream seperately to disk. If the rights for this
