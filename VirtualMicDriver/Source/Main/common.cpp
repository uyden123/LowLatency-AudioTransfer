    ...
    public CUnknown    
{
    private:
        ...
        PCVirtualMicHW   m_pHW;                  // Virtual Virtual Mic HW object
    ...
    public:
        ...
        ( 
            ...
            _In_reads_opt_(cPropertyCount) const VirtualMic_DEVPROPERTY   * pProperties,
...
NTSTATUS VirtualMicIoSetDeviceInterfacePropertyDataMultiple
(
    _In_ PUNICODE_STRING                                        SymbolicLinkName,
    _In_ ULONG                                                  cPropertyCount,
    _In_reads_opt_(cPropertyCount) const VirtualMic_DEVPROPERTY        *pProperties
...
--*/
{
    ...
    // Initialize HW.
    // 
    m_pHW = new (POOL_FLAG_NON_PAGED, VirtualMic_POOLTAG)  CVirtualMicHW;
    if (!m_pHW)
    {
        DPF(D_TERSE, ("Insufficient memory for Virtual Mic HW"));
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
    }
...
--*/
{
    ...
    //
    ntStatus = VirtualMicIoSetDeviceInterfacePropertyDataMultiple(AudioSymbolicLinkName, cPropertyCount, pProperties);

    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("CreateAudioInterfaceWithProperties: VirtualMicIoSetDeviceInterfacePropertyDataMultiple(...): failed, 0x%x", ntStatus)),
        Done);
...
)
{
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownTopology || NULL == unknownMiniTopo)
    {
        ...
        // Install Virtual Mic topology miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->TopoName, // make sure this name matches with VirtualMic.<TopoName>.szPname in the inf's [Strings] section
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownWave || NULL == unknownMiniWave)
    {
        ...
        // Install Virtual Mic wave miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->WaveName, // make sure this name matches with VirtualMic.<WaveName>.szPname in the inf's [Strings] section
