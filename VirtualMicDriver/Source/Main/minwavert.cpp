...
)
{
    ...
    if (IsEqualGUIDAligned(*PropertyRequest->PropertyItem->Set, KSPROPSETID_VirtualMic_Sideband))
    {
        switch (PropertyRequest->PropertyItem->Id)
        {
            case KSPROPERTY_SIDEBAND_PUSH_MIC_DATA:
                if (PropertyRequest->Verb & KSPROPERTY_TYPE_SET)
                {
                    // CLI triggers this to push audio data
                    if (PropertyRequest->ValueSize > 0)
                    {
                        PushMicData((PBYTE)PropertyRequest->Value, PropertyRequest->ValueSize);
                        PropertyRequest->ValueSize = 0; // Handled
                        ntStatus = STATUS_SUCCESS;
                    }
                    else
                    {
                ...
                {
                    ntStatus = PropertyHandler_BasicSupport(PropertyRequest, KSPROPERTY_TYPE_SET, VT_ILLEGAL);
                }
                break;
        }
    }
